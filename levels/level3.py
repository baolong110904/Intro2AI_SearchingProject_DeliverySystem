...
def uniform_cost_search(n, m, t, f, map_data, start, goal):
    ...
    while open_set:
        ...
        if (x, y) == goal:
            return path, time if time <= t else None
...
def solve_level_3(n, m, t, f, map_data):
    ...
    if start and goal:
        path, time_taken = uniform_cost_search(n, m, t, f, map_data, start, goal)
        if path:
...
def level_3(input_file):
    ...
    # Solve the problem
    solution, time_taken = solve_level_3(n, m, t, f, map_data)
    total_steps = len(solution[0]) - 1 if solution else 0
